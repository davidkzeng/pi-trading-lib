...
class MarketData:
    ...
    def get_raw_data(self, date: datetime.date) -> pd.DataFrame:
    ...
    @utils.copy
    @functools.lru_cache()
    def get_filtered_data(self, date: datetime.date, contracts: t.Optional[t.Tuple[int, ...]] = None,
    ...
    def get_df(self, start_date: datetime.date, end_date: datetime.date, **filter_kwargs) -> pd.DataFrame:
        """Get market data between [start_date, end_date], inclusive"""
        # TODO: Support intraday snapshots
        df = pd.concat(
            [self.get_filtered_data(date, **filter_kwargs) for date in dates.date_range(start_date, end_date)],
            axis=0
        )
